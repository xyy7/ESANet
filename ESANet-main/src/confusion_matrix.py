...
if __name__ == '__main__':
    ...
    # compute the same
    # label = np.array([0, 0, 1, 2, 2])
    # prediction = np.array([1, 1, 0, 2, 3])

    # cm_tf = ConfusionMatrixTensorflow(4)
    # cm_pytorch = ConfusionMatrixPytorch(4)

    label=np.load('/data/chenminghui/label.npy')
    prediction=np.load('/data/chenminghui/pred.npy')
    cm_tf = ConfusionMatrixTensorflow(40)
    cm_pytorch = ConfusionMatrixPytorch(40)
