...
sys.path.append("/home/xyy/ESA_backup_0720/ESANet-main")

from src import preprocessing
from src.datasets import SUNRGBD, Cityscapes, MySUNRGBD, NYUv2, SceneNetRGBD


def prepare_data(args, ckpt_dir=None, with_input_orig=False, split=None):
    train_preprocessor_kwargs = {}
    # args.dataset_dir = '/data/chenminghui/nyud/nyu5k/nyuv2'
    # args.dataset = 'nyuv2'

    if args.dataset == "sunrgbd":
        Dataset = MySUNRGBD  # 读取my_test.txt, 数据集经过重整
        # Dataset = SUNRGBD # 读取原数据集的test.txt
        dataset_kwargs = {}
        valid_set = "test"
    ...
    if args.aug_scale_min != 1 or args.aug_scale_max != 1.4:
        train_preprocessor_kwargs["train_random_rescale"] = (args.aug_scale_min, args.aug_scale_max)
    ...
    train_data = Dataset(
        ...
        rec_data_dir=args.rec_data_dir,
        **dataset_kwargs,
    )
    ...
    if ckpt_dir is not None:
        ...
        if os.path.exists(pickle_file_path):
            ...
            # dump depth stats
            depth_stats = {"mean": train_data.depth_mean, "std": train_data.depth_std}
    ...
    if args.valid_full_res:
        valid_preprocessor_full_res = preprocessing.get_preprocessor(
            depth_mean=depth_stats["mean"], depth_std=depth_stats["std"], depth_mode=depth_mode, phase="test"
        )
    ...
    batch_size_valid = 1
    args.workers = 0
    valid_loader = DataLoader(valid_data, batch_size=batch_size_valid, num_workers=args.workers, shuffle=False)

    if args.valid_full_res:
        valid_loader_full_res = copy.deepcopy(valid_loader)
        valid_loader_full_res.dataset.preprocessor = valid_preprocessor_full_res
        return None, valid_loader, valid_loader_full_res
